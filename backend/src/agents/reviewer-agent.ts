...
import { REVIEWER_AGENT_SYSTEM_PROMPT } from "@src/agents/agent-prompts";
import {
  buildAgentTools,
  buildMcpParams,
} from "@src/agents/agent-registration-options";

export async function registerReviewerAgent(): Promise<string> {
  ...
  const modelId =
    process.env.ANTHROPIC_REVIEWER_AGENT_MODEL ??
