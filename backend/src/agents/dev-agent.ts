...
import { DEV_AGENT_SYSTEM_PROMPT } from "@src/agents/agent-prompts";
import {
  buildAgentTools,
  buildMcpParams,
} from "@src/agents/agent-registration-options";

export async function registerDevAgent(): Promise<string> {
  ...
  const modelId =
    process.env.ANTHROPIC_DEV_AGENT_MODEL ??
